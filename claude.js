  ...
  async processWeatherData(weatherData) {
    try {
      ...
      for (const threshold of thresholds) {
        ...
        const isThresholdBreached = this.checkThresholdBreach(threshold, weatherData);
        ...
        if (currentCount >= threshold.consecutiveChecks) {
          ...
          const cooldownElapsed = !lastTrigger || 
          ...
          if (cooldownElapsed) {
            try {
              ...
              console.error(`Failed to create alert for threshold ${thresholdKey}:`, error);
            }
          }
        }
      }
